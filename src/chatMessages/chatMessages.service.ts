...
export class ChatMessagesService {
    constructor(@InjectModel(MESSAGE_MODEL_NAME) private _chatModel: Model<IMessage>,
    ...
    async update(messageId: string, message_update_dto: messageUpdateDTO) {
        ...
        return await this._chatModel.findOneAndUpdate({
            messageId: messageId
        }, message_update_dto, { upsert: false, new: true });
    }
    ...
    async findSelfChatMessages(token: string) {
        ...
        const found_messages = await this._chatModel.find(query).sort({ createdAt: -1 });
