...
import {
  create_session_meeting,
  delete_session_meeting,
  get_session_meeting,
} from '../common/utils/googleMeetCreator';
...
export class SessionsService {
  ...
  async createSession(token: string, session_insert_dto: sessionInsertDTO) {
    ...
    const found_package = await this.getPackageDetails(
      session_insert_dto.packageId,
    );
    if (found_package == null) {
      throw new ConflictException({ message: 'package not found' });
    }

    // Get the time frame (duration)
    const duration = await this.getPackageDuration(
      session_insert_dto.packageId,
    );
    if (duration == null) {
      throw new ConflictException({ message: 'meeting duration error' });
    }

    let google_meet_response = new googleMeetResponseDTO();
    google_meet_response = await create_session_meeting(
      session_insert_dto.bookedDate,
      session_insert_dto.startTime,
      duration,
      session_insert_dto.timeZone,
    );
  ...
  async update(sessionId: string, session_update_dto: sessionUpdateDTO) {
    ...
    let google_meet_response = new googleMeetResponseDTO();
    google_meet_response = await create_session_meeting(
      session_update_dto.bookedDate,
      session_update_dto.startTime,
      duration,
      session_update_dto.timeZone,
    );
    if (google_meet_response.busy && google_meet_response.busy == true) {
      throw new ConflictException({ message: 'busy time slot' });
    }
  ...
  async getSelfSessions(token: string) {
    ...
    query['$or'] = [{ mentorId: userId }, { developerId: userId }];
  ...
  async registerPayment(payment_insert_dto: paymentInsertDTO) {
    ...
    const result_balance = await this._userModel.findOneAndUpdate(
      {
        userId: found_session.mentorId,
        role: AppRoles.MENTOR,
      },
      { $inc: { wallletBalance: payment_insert_dto.amount } },
      { upsert: false, new: true },
    );
    const _return = {
  ...
  async findByMentorId_admin(mentor_id: string) {
    ...
    let pending_list = [];
    let new_list = [];
    const seesions_list = await this._sessionModel.find({
      mentorId: mentor_id,
    });
