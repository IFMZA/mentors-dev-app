...
export class CommentsService {
  constructor(
    @InjectModel(COMMENT_MODEL_NAME) private _commentModel: Model<IComment>,
    @InjectModel(COMMENT_LIKE_MODEL_NAME)
    private _likeModel: Model<ICommentLike>,
    @InjectModel(REPLY_MODEL_NAME) private _replyModel: Model<IReply>,
    @InjectModel(REPLY_LIKE_MODEL_NAME)
    private _replyLikeModel: Model<IReplyLike>,
  ...
  async createComment(
    base_url: string,
    token: string,
    comment_insert_dto: commentInsertDTO,
  ) {
    let userId = '';
  ...
  async delete(commentId: string) {
    const delete_result = await this._commentModel.findOneAndDelete({
      commentId: commentId,
    });
    if (delete_result) {
  ...
  async findComments(base_url: string, pageId: number) {
    ...
    const found_comments = await this._commentModel
      .find({}, {}, { skip: _skip, limit: COMMENTS_LIST_PAGE_SIZE })
      .sort({ createdAt: -1 });
    const unique_comment_ids = [
      ...new Set(found_comments.map((item) => item.commentId)),
    ];
    const found_replies = await this._replyModel
      .find(
        {
          commentId: { $in: unique_comment_ids },
        },
        {},
        { skip: _skip, limit: COMMENTS_LIST_PAGE_SIZE },
      )
      .sort({ createdAt: -1 });
    ...
    found_comments.forEach((comment) => {
      ...
      const related_replies = found_replies.filter(function (element) {
    ...
    for (let idx = 0; idx < comments_list.length; idx++) {
      ...
      else commentItem.user = { userId: '', name: 'anonymous', image: '' };
      ...
      for (let idx_ = 0; idx_ < commentItem.replies.length; idx_++) {
        ...
        const found_user_reply = await this.findUser({
          userId: commentItem.userId,
        });
        if (found_user_reply)
          _reply.user = {
            userId: found_user_reply.userId,
            name: found_user_reply.name,
            image: getImagePath(base_url, found_user_reply.profileImage),
          };
        else _reply.user = { userId: '', name: 'anonymous', image: '' };
  ...
  async likeComment(token: string, commentId: string) {
    ...
    const foundToken = await this._tokenModel.findOne({ token: token });
    userId = foundToken.userId;
    const exist_previous = await this.findOne({
      commentId: commentId,
      userId: userId,
    });
    if (exist_previous) {
  ...
  async deleteLikeComment(token: string, commentId: string) {
    ...
    const delete_result = await this._likeModel.findOneAndDelete({
      commentId: commentId,
      userId: userId,
    });
    if (delete_result) {
