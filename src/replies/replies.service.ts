...
export class RepliesService {
  ...
  async delete(replyId: string) {
    const delete_result = await this._replyModel.findOneAndDelete({
      replyId: replyId,
    });
    if (delete_result) {
  ...
  async likeReply(token: string, replyId: string, commentId: string) {
    ...
    await this._replyLikeModel.create({
      replyId: replyId,
      commentId: commentId,
      likeId: generateUUID(),
      userId: userId,
  ...
  async deleteReplyLike(token: string, replyId: string) {
    ...
    const delete_result = await this._replyLikeModel.findOneAndDelete({
      replyId: replyId,
      userId: userId,
    });
    if (delete_result) {
