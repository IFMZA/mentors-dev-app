...
export class ReviewsService {
  constructor(
    @InjectModel(REVIEW_MODEL_NAME) private _reviewModel: Model<IReview>,
  ...
  async findReviews(base_url: string, pageId: number) {
    const _skip = REVIEWS_LIST_PAGE_SIZE * pageId;
    const found_reviews = await this._reviewModel
      .find({}, {}, { skip: _skip, limit: REVIEWS_LIST_PAGE_SIZE })
      .sort({ createdAt: -1 });
    ...
    for (let idx = 0; idx < found_reviews.length; idx++) {
      ...
      else _review.user = { userId: '', name: 'anonymous', image: '' };
  ...
  async findSelfReviews(token: string, pageId: number) {
    ...
    const found_reviews = await this._reviewModel
      .find(query, {}, { skip: _skip, limit: REVIEWS_LIST_PAGE_SIZE })
      .sort({ createdAt: -1 });
  ...
  async findReviewsByMentorId(mentorId: string, pageId: number) {
    ...
    const found_reviews = await this._reviewModel
      .find(
        { mentorId: mentorId },
        {},
        { skip: _skip, limit: REVIEWS_LIST_PAGE_SIZE },
      )
      .sort({ createdAt: -1 });
    return found_reviews;
  }
