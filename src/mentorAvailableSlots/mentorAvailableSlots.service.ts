...
import {
  MENTOR_AVAILABLE_SLOT_MODEL_NAME,
  MENTOR_PACKAGE_MODEL_NAME,
  PackageTypes,
  sessionStatus,
  SESSION_MODEL_NAME,
  TOKEN_MODEL_NAME,
...
export class MentorAvailableSlotsService {
  constructor(
    @InjectModel(MENTOR_AVAILABLE_SLOT_MODEL_NAME)
    private _slotModel: Model<IMentorAvailableSlot>,
    @InjectModel(SESSION_MODEL_NAME) private _sessionModel: Model<ISession>,
    @InjectModel(MENTOR_PACKAGE_MODEL_NAME)
    private _packageModel: Model<IMentorPackage>,
  ...
  async createSlot(token: string, slot_insert_dto: slotInsertDTO) {
    ...
    isValidSlot = await this.checkSlotAvailability(
      userId,
      slot_insert_dto.availableDate,
      slot_insert_dto.fromTime,
      slot_insert_dto.ToTime,
    );
    if (!isValidSlot)
  ...
  async saveManySlots(token: string, slot_list_dto: [slotSaveManyDTO]) {
    ...
    slot_list_dto
      .filter(function (element) {
        return element.opType == OpTypes.DELETED;
      })
      .forEach(async (slot_delete_dto) => {
        const delete_response = await this._slotModel.findOneAndDelete({
          slotId: slot_delete_dto.slotId,
        });
        delete_list_response.push(delete_response);
      });

    //Check Availability before update
    const slots_to_check = slot_list_dto.filter(function (element) {
      return (
        element.opType == OpTypes.MODIFIED || element.opType == OpTypes.NEW
      );
    });
    ...
    slot_list_dto
      .filter(function (element) {
        return element.opType == OpTypes.MODIFIED;
      })
    ...
    slot_list_dto
      .filter(function (element) {
        return element.opType == OpTypes.NEW;
      })
    ...
    const insert_list_response = await this._slotModel.insertMany(
      slot_list_dto,
    );
  ...
  async update(slotId: string, slot_update_dto: slotUpdateDTO) {
    ...
    isValidSlot = await this.checkSlotAvailability(
      mentor_id,
      slot_update_dto.availableDate,
      slot_update_dto.fromTime,
      slot_update_dto.ToTime,
    );
    if (!isValidSlot)
      throw new ConflictException({
        message: `slot conflicts with another existing one ${
  ...
  async findFreeSlots(
    ...
    const busySlots = await this._sessionModel.find({
      mentorId: mentor_id,
    ...
    for (let idx = 0; idx < busySlots.length; idx++) {
      ...
      const startObj = moment
        .tz(slot_busy.startTime.toISOString(), time_zone)
        .format();
      const endObj = moment
        .tz(slot_busy.endTime.toISOString(), time_zone)
        .format();
      reserved_slots.push({ start: startObj, end: endObj });
    }
    ...
    for (let index = 0; index < available_slots.length; index++) {
      ...
      const start = moment
        .tz(
          new Date(date_segments[0] + 'T' + element.fromTime).toISOString(),
          time_zone,
        )
        .format();
      const end = moment
        .tz(
          new Date(date_segments[0] + 'T' + element.ToTime).toISOString(),
          time_zone,
        )
        .format();
      ...
      bookable.forEach((bokkable_element) => {
        bokkable_element.start = moment
          .tz(bokkable_element.start, time_zone)
          .format();
        bokkable_element.end = moment
          .tz(bokkable_element.end, time_zone)
          .format();
        if (
          free_slots.filter(
            (e) =>
              e.start === bokkable_element.start &&
              e.end === bokkable_element.end,
          ).length == 0
        )
